...
1. 先查看 [GitHub Releases](https://github.com/jjy1226609858-ui/archscope-codex/releases)，下载 `ArchScope-v0.11.7-windows-x64.zip`，并用 `SHA256SUMS.txt` 核对 SHA-256。如果尚无 Release，登录 GitHub，打开 **Actions → Windows CI → 最新成功的 `main` 运行 → Artifacts → `ArchScope-v0.11.7-windows-x64`** 下载临时包；Actions 产物 30 天后过期。解压到准备长期保留的目录，确认有 `.agents/plugins/marketplace.json` 和 `plugins/archscope/`，不要运行来源不明的程序。
