...
请先查看 [GitHub Releases](https://github.com/jjy1226609858-ui/archscope-codex/releases) 是否已有按版本命名的 Windows ZIP 和 SHA-256 校验值。如果尚无 Release，已登录 GitHub 的用户可到 **Actions → Windows CI → 最新成功的 `main` 运行 → Artifacts** 下载临时包。CI 仅在测试、隐私扫描、打包验证和示例运行通过后上传；Actions 产物 30 天后过期。这两种下载方式都不等于受保护的外部签名验收。
